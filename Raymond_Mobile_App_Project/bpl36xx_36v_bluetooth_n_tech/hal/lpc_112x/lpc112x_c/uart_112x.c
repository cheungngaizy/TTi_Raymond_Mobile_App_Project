 ...
 * Private functions
 ****************************************************************************/
// add getUARTClockID by WPI Wecan Chen 2017.02.20 
/* Return UART clock ID from the UART register address */
static CHIP_SYSCON_CLOCK_T getUARTClockID(LPC_UART_T *pUART)
{
	if (pUART == LPC_UART0) {
...
void Chip_UART_Init(LPC_UART_T *pUART)
{
	...
	//Chip_Clock_SetUARTClockDiv(1); remove by WPI Wecan Chen 2017.02.20
...
{
	Chip_Clock_DisablePeriphClock(getUARTClockID(pUART));	// Modify by WPI Wecan Chen 2017.03.08
	//Chip_Clock_DisablePeriphClock(SYSCON_CLOCK_UART0); remove by WPI Wecan Chen 2017.03.08
}
...
void Chip_UART_RXIntHandlerRB(LPC_UART_T *pUART, RINGBUFF_T *pRB)
{
    ...
    RingBuf_Write(pRB, &ch, 1); // RingBuffer_Insert(pRB, &ch); remove by WPI Wecan Chen 2017.02.22
	}
}
...
void Chip_UART_TXIntHandlerRB(LPC_UART_T *pUART, RINGBUFF_T *pRB)
{
    ...
    // RingBuffer_Pop(pRB, &ch)) { remove by WPI Wecan Chen 2017.02.22
...
uint32_t Chip_UART_SendRB(LPC_UART_T *pUART, RINGBUFF_T *pRB, const void *data, int bytes)
{
  ...
  ret = RingBuf_Write(pRB, p8, bytes);
	//ret = RingBuffer_InsertMult(pRB, p8, bytes); remove by WPI Wecan Chen 2017.02.22
	Chip_UART_TXIntHandlerRB(pUART, pRB);

	/* Add additional data to transmit ring buffer if possible */
  ret += RingBuf_Write(pRB, (p8 + ret), (bytes - ret));
	//ret += RingBuffer_InsertMult(pRB, (p8 + ret), (bytes - ret)); remove by WPI Wecan Chen 2017.02.22
...
int Chip_UART_ReadRB(LPC_UART_T *pUART, RINGBUFF_T *pRB, void *data, int bytes)
{
	...
	//return RingBuffer_PopMult(pRB, (uint8_t *) data, bytes); remove by WPI Wecan Chen 2017.02.22
}
...
void Chip_UART_IRQRBHandler(LPC_UART_T *pUART, RINGBUFF_T *pRXRB, RINGBUFF_T *pTXRB)
{
	...
	if (pUART->IER & UART_IER_THREINT) {
		...
		/* Disable transmit interrupt if the ring buffer is empty */
			if (RingBuf_GetUsedBytes(pTXRB) == 0) {
			//if (RingBuffer_IsEmpty(pTXRB)) { remove by WPI Wecan Chen 2017.02.22
			Chip_UART_IntDisable(pUART, UART_IER_THREINT);
		}
	}
